...
typedef struct {
    node_t **treeArray;
    uint_fast32_t numNodes;
    uint_fast32_t maxNodes;
...
typedef struct {
    uint_fast8_t symbol;
    uint_fast16_t bitcode;
    uint_fast32_t code_length;
...
node_t* allocateNewNode(             // Dynamically allocates a min-heap node containing a symbol-probability pair
    uint_fast8_t symbol,
    float probability);
uint_fast32_t isLeaf(                // Returns 1 if the given node is a leaf node; returns -1 otherwise
    node_t* node);
...
void lutPopulate(                     // 
    ...
    uint_fast8_t* buff, 
    uint_fast32_t bit_count, 
    uint_fast32_t longest_code_exp,
    lut** all_luts, 
    uint_fast32_t* lut_num, 
    uint_fast32_t* row_count);
void lutCreation(                     // 
    node_t* root, 
    lut** all_luts, 
    uint_fast32_t* tables_needed, 
    uint_fast32_t longest_code_exp);
